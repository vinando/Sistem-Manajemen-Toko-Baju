 ...
 */
class Application_Model_Trnfinishorder {
    private $sDb;
    ...
    function getSingle($strTrnID) {
        $strSQL = "SELECT * FROM trn_finish_order WHERE id = '$strTrnID'";
    ...
    function getAll($intLimit = 0, $intOffset = 50) {
        $strSQL = "SELECT * FROM trn_finish_order LIMIT $intLimit, $intOffset";
        $stmt = $this->sDb->query($strSQL);
        $stmt->setFetchMode(Zend_Db::FETCH_BOTH);
        $res = $stmt->fetchAll();
        return $res;
    ...
    function insert($strBranchID, $strFinishDate, $strFinishNo, $strOrderId, $strOrderNo, $strEmployeeId, $strEmployeeName, $strPrice) {
        $strSQL = "INSERT INTO trn_finish_order (branchid, finishdate, finishno, orderid, orderno, employeeid, employeename, price) VALUES ('$strBranchID', '$strFinishDate', '$strFinishNo', '$strOrderId', '$strOrderNo', '$strEmployeeId', '$strEmployeeName', '$strPrice')";
        $stmt = $this->sDb->query($strSQL);
        return $this->sDb->lastInsertId();
    }

    function update($strFinishId, $strEmployeeId, $strEmployeeName, $strPrice) {
        $strSQL = " UPDATE trn_finish_order
                            SET
    ...
    function updateFinishNo($finish_id, $finish_no) {
        $strSQL = "UPDATE trn_finish_order SET finishno = '$finish_no' WHERE id = '$finish_id'";
        $stmt = $this->sDb->query($strSQL);
    }

    function delete($strTrnID) {
        //Delete transaction
        $strSQL = "DELETE FROM trn_finish_order WHERE id = '$strTrnID'";
        $this->sDb->query($strSQL);
    }

    function getDataTable() {
        $sTable   = "trn_finish_order";
        $aColumns = array( 'id', 'branchid', 'finishdate', 'finishno', 'orderid', 'orderno', 'employeeid', 'employeename', 'price');
        $sIndexColumn = "id";
