...
class Application_Model_Employee {
	...
	function getAll($intLimit = 0, $intOffset = 50) {
		$strSQL = "SELECT * FROM mst_employee LIMIT $intLimit, $intOffset";
    ...
    function getAllByBranchID($BranchID) {
        $strSQL = "SELECT * FROM mst_employee WHERE branchid = '$BranchID'";
        $stmt = $this->sDb->query($strSQL);
        $stmt->setFetchMode(Zend_Db::FETCH_BOTH);
        $res = $stmt->fetchAll();
        return $res;
    ...
    function getAllKaryawanByBranchID($BranchID) {
        $strSQL = " SELECT *
                    FROM mst_employee
                    WHERE
                        branchid = '$BranchID' AND
                        (type = 'Back Office' OR type = 'SPG')";
        $stmt = $this->sDb->query($strSQL);
        $stmt->setFetchMode(Zend_Db::FETCH_BOTH);
        $res = $stmt->fetchAll();
        return $res;
	...
	function getAllByEmployeeID($strEmployeeID) {
		if ($strEmployeeID) {
			$strSQL = "SELECT * FROM mst_employee WHERE id = '$strEmployeeID'";
		} else {
			$strSQL = "SELECT * FROM mst_employee";
		}
		$stmt = $this->sDb->query($strSQL);
		$stmt->setFetchMode(Zend_Db::FETCH_BOTH);
		$res = $stmt->fetchAll();
	...
	/**
	 * Get all EmployeeID in an array
	 * @return Array EmployeeID
	 */
	function getAllEmployeeID() {
		$strSQL = "SELECT id FROM mst_employee";
		$stmt = $this->sDb->query($strSQL);
		$stmt->setFetchMode(Zend_Db::FETCH_BOTH);
		$res = $stmt->fetchAll();
		return $res;
	...
	function getSingle($strEmployeeID) {
		$strSQL = "SELECT * FROM mst_employee WHERE id = '$strEmployeeID'";
		$stmt = $this->sDb->query($strSQL);
		$stmt->setFetchMode(Zend_Db::FETCH_BOTH);
		$res = $stmt->fetch();
	...
	function delete($strEmployeeID) {
        $strSQL = "DELETE FROM mst_employee WHERE id = '$strEmployeeID'";
        $stmt = $this->sDb->query($strSQL);
	}

	function getDataTable() {
	    $sTable   = "mst_employee";
	    $aColumns = array( 'id', 'branchid', 'type', 'code', 'name', 'dob', 'gender', 'salary');
	    $sIndexColumn = "id";
