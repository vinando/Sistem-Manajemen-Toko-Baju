 ...
 */
class Application_Model_Trnpenggunaanbahan {
    private $sDb;
    ...
    function getSingle($strTrnId) {
        $strSQL = "SELECT * FROM trn_material_using WHERE id = '$strTrnId'";
    ...
    function getAll($intLimit = 0, $intOffset = 50) {
        $strSQL = "SELECT * FROM trn_material_using LIMIT $intLimit, $intOffset";
        $stmt = $this->sDb->query($strSQL);
        $stmt->setFetchMode(Zend_Db::FETCH_BOTH);
        $res = $stmt->fetchAll();
        return $res;
    ...
    function getTrnDetails($trn_id) {
        $strSQL = " SELECT t1.*, t2.code as productcode, t2.name as productname
                    FROM trn_material_using_dt t1, mst_product t2
    ...
    function updateTrnNo($trn_id, $trn_no) {
        $strSQL = "UPDATE trn_material_using SET trnno = '$trn_no' WHERE id = '$trn_id'";
        $stmt = $this->sDb->query($strSQL);
    }

    function insertHeader($strBranchID, $strOrderId, $strOrderNo, $strTrnNo, $strTrnDate) {
        $strSQL = "INSERT INTO trn_material_using (branchid, orderid, orderno, trnno, trndate) VALUES ('$strBranchID', '$strOrderId', '$strOrderNo', '$strTrnNo', '$strTrnDate')";
        $stmt = $this->sDb->query($strSQL);
        return $this->sDb->lastInsertId();
    }

    function insertDetail($strTrnId, $strProductId, $strQty) {
        $strSQL = "INSERT INTO trn_material_using_dt (trnid, productid, amount) VALUES ('$strTrnId', '$strProductId', '$strQty')";
        $stmt = $this->sDb->query($strSQL);
    }

    function updateHeader($strTrnId, $strBranchId, $strOrderId, $strOrderNo, $strTrnNo, $strTrnDate) {
        $strSQL = " UPDATE trn_material_using
                    SET
    ...
    function delete($strTrnId) {
        ...
        $strSQL = "DELETE FROM trn_material_using WHERE id = '$strTrnId'";
        $this->sDb->query($strSQL);
    }

    function deleteDetail($strTrnId) {
        $strSQL = "DELETE FROM trn_material_using_dt WHERE trnid = '$strTrnId'";
        $this->sDb->query($strSQL);
    }
    ...
    function getDataTable() {
        $sTable   = "trn_material_using";
        $aColumns = array( 'id', 'branchid', 'trnno', 'orderno', 'trndate');
        $sIndexColumn = "id";
