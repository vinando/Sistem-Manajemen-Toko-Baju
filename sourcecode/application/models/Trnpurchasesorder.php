 ...
 */
class Application_Model_Trnpurchasesorder {
    private $sDb;
    ...
    function getSingle($strOrderID) {
        $strSQL = "SELECT * FROM trn_purchases_order WHERE id = '$strOrderID'";
    ...
    function getAll($intLimit = 0, $intOffset = 50) {
        $strSQL = "SELECT * FROM trn_purchases_order LIMIT $intLimit, $intOffset";
        $stmt = $this->sDb->query($strSQL);
        $stmt->setFetchMode(Zend_Db::FETCH_BOTH);
        $res = $stmt->fetchAll();
        return $res;
    ...
    function getOrderDetails($order_id) {
        $strSQL = " SELECT t1.*, t2.code as productcode, t2.name as productname, t3.name as sizename, t4.name as colorname, t5.name as materialname
                    FROM trn_purchases_order_dt t1  LEFT JOIN mst_product t2 ON t1.productid = t2.id
    ...
    function updateOrderNo($order_id, $order_no) {
        $strSQL = "UPDATE trn_purchases_order SET orderno = '$order_no' WHERE id = '$order_id'";
        $stmt = $this->sDb->query($strSQL);
    }
    ...
    function insertHeader($strBranchID, $strOrderNo, $strOrderDate, $strSupplierId, $strSupplierName, $strPhone) {
        $strSQL = "INSERT INTO trn_purchases_order (branchid, orderno, orderdate, supplierid, suppliername, phone) VALUES ('$strBranchID', '$strOrderNo', '$strOrderDate', '$strSupplierId', '$strSupplierName', '$strPhone')";
        $stmt = $this->sDb->query($strSQL);
    ...
    function insertDetail($strOrderID, $strProductID, $strNoSeri, $strSizeID, $strColorID, $strMaterialID, $strPrice, $strQty, $strTotal) {
        $strSQL = "INSERT INTO trn_purchases_order_dt (orderid, productid, noseri, sizeid, colorid, materialid, price, amount, total) VALUES ('$strOrderID', '$strProductID', '$strNoSeri', '$strSizeID', '$strColorID', '$strMaterialID', '$strPrice', '$strQty', '$strTotal')";
        $stmt = $this->sDb->query($strSQL);
    }

    function updateHeader($strOrderId, $strSupplierId, $strSupplierName, $strPhone) {
        $strSQL = "UPDATE trn_purchases_order SET supplierid = '$strSupplierId', suppliername = '$strSupplierName', phone = '$strPhone' WHERE id = '$strOrderId'";
        $stmt = $this->sDb->query($strSQL);
    }
    ...
    function delete($strOrderID) {
        ...
        $strSQL = "DELETE FROM trn_purchases_order WHERE id = '$strOrderID'";
        $this->sDb->query($strSQL);
    }

    function deleteDetail($strOrderID) {
        $strSQL = "DELETE FROM trn_purchases_order_dt WHERE orderid = '$strOrderID'";
        $this->sDb->query($strSQL);
    }
    ...
    function insertStock($strBranchId, $strProductId, $strNoSeri, $qty) {
        $strSQL = "INSERT INTO mst_stock VALUES ('', '$strBranchId', '$strProductId', '$strNoSeri', '$qty', '')";
        //return $strSQL;
        $this->sDb->query($strSQL);
    }

    function deleteStock($strBranchId, $strProductId, $strNoSeri) {
        $strSQL = "DELETE FROM mst_stock WHERE branchid = '$strBranchId' AND productid = '$strProductId' AND noseri = '$strNoSeri'";
        //return $strSQL;
        $this->sDb->query($strSQL);
    }

    function getDataTable() {
        $sTable   = "trn_purchases_order";
        $aColumns = array( 'id', 'branchid', 'orderno', 'orderdate', 'suppliername', 'phone');
        $sIndexColumn = "id";
