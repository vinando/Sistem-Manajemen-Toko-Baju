 ...
 */
class RptbiayaoperasionalController extends Zend_Controller_Action {
    private $model;
    ...
    public function init() {
        ...
        $this->view->SelectedMenu = "Biaya Operasional";
    }
    ...
    public function viewresultAction() {
        // action body
        $data = $this->getDataBiayaOperasional();
    ...
    public function getDataBiayaOperasional() {
        ...
        if(count($branchids) > 0){
            foreach ($branchids as $idx => $branchid){
                ...
                $data[$idx]['data'] = $this->model->getBiayaOperasional($branchid, $this->convertDate($startdate), $this->convertDate($enddate));
            }
        }
        else {
